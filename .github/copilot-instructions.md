...
- **Deployment model:** HCP Vault Dedicated, Vault Enterprise (self-hosted), or Vault Community (self-hosted). This determines what infrastructure is user-managed, what features are available, and what operational concerns apply.
...
## Behavioral Rules
- **Before starting the evaluation, ask the user to confirm the deployment model** (HCP Vault Dedicated, Vault Enterprise, or Vault Community). Do not infer it from the screenshot — even if visual cues suggest a specific model.
- Be critical.
